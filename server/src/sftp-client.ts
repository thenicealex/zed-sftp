...
import { ProxyConfig, SftpConfig, ConfigManager } from './config';
...
export class SftpClient {
  ...
  private async createProxySocket(): Promise<net.Socket | null> {
  ...
  private async connectToProxy(proxy: ProxyConfig): Promise<net.Socket> {
  ...
  private async establishHttpTunnel(socket: net.Socket, proxy: ProxyConfig): Promise<void> {
    ...
    const response = await this.readSocketUntil(
      socket,
      (buffer) => buffer.indexOf('\r\n\r\n') !== -1,
      'Failed to establish HTTP proxy tunnel',
    );
  ...
  private async establishSocks5Tunnel(socket: net.Socket, proxy: ProxyConfig): Promise<void> {
    ...
    const negotiation = await this.readSocketBytes(
      socket,
      2,
      'Failed to negotiate SOCKS5 authentication',
    );
    ...
    const replyHeader = await this.readSocketBytes(
      socket,
      4,
      'Failed to establish SOCKS5 proxy tunnel',
    );
    ...
    await this.consumeSocks5ReplyAddress(
      socket,
      replyHeader[3],
      'Failed to establish SOCKS5 proxy tunnel',
    );
  ...
  private async performSocks5Authentication(socket: net.Socket, proxy: ProxyConfig): Promise<void> {
    ...
    const authReply = await this.readSocketBytes(
      socket,
      2,
      'Failed to negotiate SOCKS5 authentication',
    );
  ...
  private createSocks5Address(host: string): Buffer {
    if (net.isIP(host) === 4) {
  ...
  private async consumeSocks5ReplyAddress(
    socket: net.Socket,
    addressType: number,
    context: string,
  ): Promise<void> {
  ...
  private async writeToSocket(socket: net.Socket, chunk: string | Buffer): Promise<void> {
  ...
  private async readSocketBytes(
    ...
    const response = await this.readSocketUntil(
      socket,
      (buffer) => buffer.length >= minimumBytes,
      context,
    );
  ...
  private formatAuthority(host: string, port: number): string {
    return net.isIP(host) === 6 ? `[${host}]:${port}` : `${host}:${port}`;
  }

  private getSocks5ReplyError(code: number): string {
    const replyMessages: Record<number, string> = {
  ...
  private getExpectedHostFingerprint(): { algorithm: 'sha256' | 'md5'; value: string } | null {
